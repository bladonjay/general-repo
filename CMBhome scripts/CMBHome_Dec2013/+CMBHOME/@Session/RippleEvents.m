...
gaussian_window = gausswin(round(.050*self.fs)); % 50 ms gaussian window

min_duration = .020; % seconds, minimum ripple duration

ripple_envelope = sqrt( self.InstantaneousPower(ind, 'Passband', 'ripple') ); % ripple envelope
...
ripple_events(ripple_events(:,2)-ripple_events(:,1)<min_duration*self.fs,:) = []; % remove ripples with lower crossing duration less than min_duration